...
const ItemProductComponent = ({ product }) => {
  ...
  const minPrice = Math.min(
    ...product.sizeColorProductsEntity.map((item) => item.listPrice)
  );
  const minDiscountPrice = Math.min(
    ...product.sizeColorProductsEntity.map((item) => item.discountPrice)
  );
  const minDiscount = Math.min(
    ...product.sizeColorProductsEntity.map((item) => item.discount)
  );
  return (
          ...
          >
            <div className="product-thumbnail pos-relative">
              ...
              >
                ...
                />
                {product.imageProductEntity.length > 1 &&
                  product.imageProductEntity[1] && (
                    <img
                      loading="lazy"
              ...
              <div className="label_product d-none">
                <div className="label_wrapper">- {minDiscount}%</div>
              </div>
              ...
              <div className="product-item-cta position-relative">
                <div className="price-box">
                  <span className="price product-price">
                    {parseInt(minDiscountPrice).toLocaleString("it-IT")}₫
                  </span>
                  <span className="compare-price">
                    {parseInt(minPrice).toLocaleString("it-IT")}₫
                  </span>
