package site.sugarnest.backend.JWT;
...
public class JwtUtils {
    ...
    public ResponseCookie generateJwtCookie(AccountEntity userPrincipal) {
        String jwt = generateTokenFromUsername(userPrincipal.getAccountName());
        return ResponseCookie.from(jwtCookie, jwt).path("/api").maxAge(24 * 60 * 60).httpOnly(true).build();
    }

    public ResponseCookie generateJwtCookie(Account user) {
        String jwt = generateTokenFromUsername(user.toString());
